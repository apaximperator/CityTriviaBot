...
def get_info(message):
    bot.reply_to(message, 'Этот бот создан для игры в города.\nСтран в базе: {0}.\nПользователей в игре: {1}.'.format(
        len(config.all_domains.items()), len(in_game)))
...
def welcome(message):
    ...
    if message.chat.id not in first_message:
        ...
        bot.send_message(message.chat.id, 'Привет, {0}! Давай сыграем в города?'.format(message.chat.first_name),
                         reply_markup=markup)
...
def main(message):
    ...
    def result(a=0):
        ...
        if int(result) == 0:
            ...
            res = (str(result) + ' городов! Я уже думал, что ты меня победишь! Я знаю всего лишь около ' + str(
                len(config.cities)) + ' городов')
        if a != 0:
            return str(result)
        else:
            return (res)
    ...
    if message.chat.id not in first_message:
        ...
        if message.chat.id in in_game:
                        ...
                        if message.text in cities:
                            if first_letter(message.text).upper() == last_letter(last_city[message.chat.id]).upper():
                                ...
                                if len(available_cities) != 0:
                                    ...
                                    bot.send_message(message.chat.id, 'Мне нечего ответить. Твоя победа.',
                                                     parse_mode='html')
                                    replay_keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
                                    button1 = types.KeyboardButton('Играть снова')
                                    replay_keyboard.add(button1)
                                    bot.send_message(message.chat.id,
                                                     'Тебе удалось победить меня, количество названных городов: ' + result(
                                                         1), reply_markup=replay_keyboard)
                                    over_game()
                            else:
                                bot.send_message(message.chat.id,
                                                 'Не совпадает - {0} и {1}'.format(message.text[0].upper(),
                                                                                   last_city[message.chat.id][
                                                                                       -1].upper()), parse_mode='html')
                        else:
                            bot.send_message(message.chat.id,
                                             'Такого города нет в выбранных тобой странах или вообще не существет')
        else:
            if message.chat.id not in user_chosen_countries:
                ...
                bot.send_message(message.chat.id,
                                 'Замечательно, теперь тебе нужно выбрать страны, города в которых будут использоваться в игре.'.format(
                                     message.chat.first_name), reply_markup=countries_markup())
            else:
                if len(user_chosen_countries[message.chat.id]) != 0 and message.text == 'Готово':
                    ...
                    bot.send_message(message.chat.id,
                                     'Отлично, страны выбраны, теперь можно начинать!'.format(message.chat.first_name),
                                     reply_markup=markup)
                    in_game[message.chat.id] = False
                elif len(user_chosen_countries[message.chat.id]) == 0 and message.text == 'Готово':
                    bot.send_message(message.chat.id,
                                     'Нужно выбрать хотябы одну страну для игры'.format(message.chat.first_name))
    ...
    try:
        if call.message:
            ...
            if country not in user_chosen_countries[call.message.chat.id] and call.message.chat.id not in in_game:
                user_chosen_countries[call.message.chat.id] = user_chosen_countries[call.message.chat.id] + [
                    call.data.split(' ')[0]]
                bot.send_message(call.message.chat.id, 'Города из {0} добавлены! (+{1})'.format(call.data.split(' ')[1],
                                                                                                len(config.all_domains[
                                                                                                        call.data.split(
                                                                                                            ' ')[0]])),
                                 reply_markup=markup)
                ...
                bot.send_message(call.message.chat.id, 'Города из {0} убраны (-{1})'.format(call.data.split(' ')[1],
                                                                                            len(config.all_domains[
                                                                                                    call.data.split(
                                                                                                        ' ')[0]])),
                                 reply_markup=markup)
